--nodemcu.readthedocs.io/en/master/en/modules/bme280/
--[[
...
If you like this project, please add a star!
	...
	------------------------------------------                  BME280  Module ------------------------------------------------------------
	-----------------------------------------------------------------------------------------------------------------------------------
local BME280 = ...			-- Obtain a sample value from somewhere
local BME280_Module =  {}	-- construct an empty table and assign it to variable "Table_name" (ex: local Arun =  {})
_G[BME280] = BME280_Module	--	A global variable (not a function) that holds the global environment 


function BME280_Module
--[[	bme280.altitude()	-	For given air pressure and sea level air pressure returns the altitude in meters as an integer multiplied with 100, i.e. altimeter function.
	Syntax		-	bme280.altitude(P, QNH)
	Parameters	-	P measured pressure
...
function BME280_Module
--[[	bme280.baro()	-	Reads the sensor and returns the air pressure in hectopascals as an integer multiplied with 1000 or nil when readout is not successful. Current temperature is needed to calculate the air pressure so temperature reading is performed prior reading pressure data. Second returned variable is therefore current air temperature.
	Syntax		-	bme280.baro()
	Parameters	-	none
	Returns		-	P air pressure in hectopascals multiplied by 1000
					T temperature in celsius as an integer multiplied with 100
]]--
end

function BME280_Module
--[[	bme280.dewpoint()	-		For given temperature and relative humidity returns the dew point in celsius as an integer multiplied with 100.
	Syntax		-	bme280.dewpoint(H, T)
	Parameters	-	H relative humidity in percent multiplied by 1000.
...
--[[	bme280.humi()	-	Reads the sensor and returns the air relative humidity in percents as an integer multiplied with 100 or nil when readout is not successful. Current temperature is needed to calculate the relative humidity so temperature reading is performed prior reading pressure data. Second returned variable is therefore current temperature.
	Syntax		-		bme280.humi()
	Parameters	-	none
	Returns		-	H last relative humidity reading in % times 1000
					T temperature in celsius as an integer multiplied with 100
]]--
end

function BME280_Module
--[[	bme280.init()	-	Initializes module. Initialization is mandatory before read values.
	Syntax		-	bme280.init(sda, scl, [temp_oss, press_oss, humi_oss, power_mode, inactive_duration, IIR_filter])
	Parameters	-	sda - SDA pin
					scl - SCL pin
...
function BME280_Module
--[[	bme280.qfe2qnh()	-	For given altitude converts the air pressure to sea level air pressure.
...
function BME280_Module
--[[	bme280.startreadout()	-	Starts readout (turns the sensor into forced mode). After the readout the sensor turns to sleep mode.
...
function BME280_Module
--[[	bme280.temp()	-	Reads the sensor and returns the temperature in celsius as an integer multiplied with 100.
	Syntax		-	bme280.temp()
	Parameters	-	none
	Returns		-	T temperature in celsius as an integer multiplied with 100 or nil when readout is not successful
					t_fine temperature measure used in pressure and humidity compensation formulas (generally no need to use this value)
]]--
end


return BME280_Module