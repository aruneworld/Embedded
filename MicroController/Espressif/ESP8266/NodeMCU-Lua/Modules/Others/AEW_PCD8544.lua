...
function M.LcdSetPins(sclk, sdin, dc, sce, reset)
	PIN_SCLK = sclk
	PIN_SDIN = sdin
	PIN_DC = dc
	PIN_SCE = sce
...
function M.LcdLocate(x,y)
	LCDWrite(LCD_C,64+x) 
	LCDWrite(LCD_C,128+y)
end

function M.LcdCharacter(character)
	LCDWrite(LCD_D, 0x00)
	for index = 0,5 do
...
function M.LcdPrintStr(str)
	local i
	for i = 1, #str do
	    M.LcdPrintChar(str:sub(i,i))
	end
end		
